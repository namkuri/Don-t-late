...
| 요청 길이 | 0.9s |
| dest | `Assets/Audio/SFX/sfx_level_up.wav` |

## 창작 태그 (사람이 고치는 유일한 칸)

<!-- NOTE:BEGIN -->
short level up chime, three quick ascending notes, bright cheerful chiptune arpeggio, tiny sparkle tail
<!-- NOTE:END -->
...
<!-- PROMPT:BEGIN -->
```
short level up chime, three quick ascending notes, bright cheerful chiptune arpeggio, tiny sparkle tail. Duration about 0.9 seconds. Style: cozy cute toy-like game sound, soft wooden marimba and rounded synth plucks, playful little pitch bends, gentle and warm, light and bouncy. Single isolated sound effect, dry and close, no background music, no vocals, no long reverb tail.
```
<!-- PROMPT:END -->
...
```bash
python scripts/audio/prompt_builder.py build --bom-id sfx_level_up
python scripts/audio/elevenlabs_client.py gen  --bom-id sfx_level_up
python scripts/audio/audio_pipeline.py intake --bom-id sfx_level_up
```
