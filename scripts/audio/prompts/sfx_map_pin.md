...
| 요청 길이 | 0.5s |
| dest | `Assets/Audio/SFX/sfx_map_pin.wav` |

## 창작 태그 (사람이 고치는 유일한 칸)

<!-- NOTE:BEGIN -->
one tiny bright map pin pop, single bouncy toy plink, snappy
<!-- NOTE:END -->
...
```bash
python scripts/audio/prompt_builder.py build --bom-id sfx_map_pin
python scripts/audio/elevenlabs_client.py gen  --bom-id sfx_map_pin
python scripts/audio/audio_pipeline.py intake --bom-id sfx_map_pin
```
