...
```bash
python scripts/audio/prompt_builder.py build --bom-id sfx_pickup
# ↑ 전송 프롬프트로 MCP 생성 → _audio_intake/elevenlabs/ 착지
python scripts/audio/audio_pipeline.py intake    --bom-id sfx_pickup
python scripts/audio/audio_pipeline.py normalize --bom-id sfx_pickup
python scripts/audio/audio_pipeline.py promote   --bom-id sfx_pickup --yes   # ⚠ 별도 승인
```
