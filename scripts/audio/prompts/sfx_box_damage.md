...
| 항목 | 값 |
|---|---|
| bom_id | `sfx_box_damage` |
| 종류 | SFX |
| 트리거 | (BOM 미등재) |
| 소리 | (BOM 미등재 — 생성은 규격만으로 진행) |
| 요청 길이 | 0.5s |
| dest | `Assets/Audio/SFX/sfx_box_damage.wav` |
...
```bash
python scripts/audio/prompt_builder.py build --bom-id sfx_box_damage
python scripts/audio/elevenlabs_client.py gen  --bom-id sfx_box_damage
python scripts/audio/audio_pipeline.py intake --bom-id sfx_box_damage
```
