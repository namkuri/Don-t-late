...
| 항목 | 값 |
|---|---|
| bom_id | `sfx_settle_bad` |
| 종류 | SFX |
| 트리거 | (BOM 미등재) |
| 소리 | (BOM 미등재 — 생성은 규격만으로 진행) |
| 요청 길이 | 1.5s |
| dest | `Assets/Audio/SFX/sfx_settle_bad.wav` |
...
```bash
python scripts/audio/prompt_builder.py build --bom-id sfx_settle_bad
python scripts/audio/elevenlabs_client.py gen  --bom-id sfx_settle_bad
python scripts/audio/audio_pipeline.py intake --bom-id sfx_settle_bad
```
