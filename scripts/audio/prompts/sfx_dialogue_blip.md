...
| 항목 | 값 |
|---|---|
| bom_id | `sfx_dialogue_blip` |
| 종류 | SFX |
| 트리거 | 대화 글자 진행 |
| 소리 | 포켓몬식 블립 |
| 요청 길이 | 0.5s |
| dest | `Assets/Audio/SFX/sfx_dialogue_blip.wav` |

## 창작 태그 (사람이 고치는 유일한 칸)

<!-- NOTE:BEGIN -->
single 8-bit text blip, one very short square-wave tick
<!-- NOTE:END -->
...
<!-- PROMPT:BEGIN -->
```
single 8-bit text blip, one very short square-wave tick. Duration about 0.5 seconds. Style: 8-bit retro game sound design, chiptune sound chip, square wave and noise channel, subtle and understated, dark comedy tone, readable in a busy mix. Single isolated sound effect, dry and close, no background music, no vocals, no long reverb tail.
```
<!-- PROMPT:END -->
...
```bash
python scripts/audio/prompt_builder.py build --bom-id sfx_dialogue_blip
python scripts/audio/elevenlabs_client.py gen  --bom-id sfx_dialogue_blip
python scripts/audio/audio_pipeline.py intake --bom-id sfx_dialogue_blip
```
