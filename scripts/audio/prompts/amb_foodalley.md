...
| 항목 | 값 |
|---|---|
| bom_id | `amb_foodalley` |
| 종류 | SFX |
| 트리거 | (BOM 미등재) |
| 소리 | (BOM 미등재 — 생성은 규격만으로 진행) |
| 요청 길이 | 5.0s |
| dest | `Assets/Audio/SFX/amb_foodalley.wav` |

## 창작 태그 (사람이 고치는 유일한 칸)

<!-- NOTE:BEGIN -->
bustling night food alley ambience, lively crowd chatter, sizzling frying sounds, clinking glasses, seamless loop, ambient bed, distant
<!-- NOTE:END -->
...
<!-- PROMPT:BEGIN -->
```
bustling night food alley ambience, lively crowd chatter, sizzling frying sounds, clinking glasses, seamless loop, ambient bed, distant. Duration about 5.0 seconds. Style: cozy cute toy-like game sound, soft wooden marimba and rounded synth plucks, playful little pitch bends, gentle and warm, light and bouncy. Single isolated sound effect, dry and close, no background music, no vocals, no long reverb tail.
```
<!-- PROMPT:END -->
...
```bash
python scripts/audio/prompt_builder.py build --bom-id amb_foodalley
python scripts/audio/elevenlabs_client.py gen  --bom-id amb_foodalley
python scripts/audio/audio_pipeline.py intake --bom-id amb_foodalley
```
