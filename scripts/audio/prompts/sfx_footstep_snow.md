...
| 항목 | 값 |
|---|---|
| bom_id | `sfx_footstep_snow` |
| 종류 | SFX |
| 트리거 | (BOM 미등재) |
| 소리 | (BOM 미등재 — 생성은 규격만으로 진행) |
| 요청 길이 | 0.5s |
| dest | `Assets/Audio/SFX/sfx_footstep_snow.wav` |

## 창작 태그 (사람이 고치는 유일한 칸)

<!-- NOTE:BEGIN -->
crunchy snow step, quick powdery squeak, soft press
<!-- NOTE:END -->
...
```bash
python scripts/audio/prompt_builder.py build --bom-id sfx_footstep_snow
python scripts/audio/elevenlabs_client.py gen  --bom-id sfx_footstep_snow
python scripts/audio/audio_pipeline.py intake --bom-id sfx_footstep_snow
```
