...
| 항목 | 값 |
|---|---|
| bom_id | `sfx_settle_ok` |
| 종류 | SFX |
| 트리거 | (BOM 미등재) |
| 소리 | (BOM 미등재 — 생성은 규격만으로 진행) |
| 요청 길이 | 1.5s |
| dest | `Assets/Audio/SFX/sfx_settle_ok.wav` |

## 창작 태그 (사람이 고치는 유일한 칸)

<!-- NOTE:BEGIN -->
end of day settlement complete, warm rising toy marimba flourish, three quick ascending wooden notes, satisfied and rewarding
<!-- NOTE:END -->
...
<!-- PROMPT:BEGIN -->
```
end of day settlement complete, warm rising toy marimba flourish, three quick ascending wooden notes, satisfied and rewarding. Duration about 1.5 seconds. Style: cozy cute toy-like game sound, soft wooden marimba and rounded synth plucks, playful little pitch bends, gentle and warm, light and bouncy. Single isolated sound effect, dry and close, no background music, no vocals, no long reverb tail.
```
<!-- PROMPT:END -->
...
```bash
python scripts/audio/prompt_builder.py build --bom-id sfx_settle_ok
python scripts/audio/elevenlabs_client.py gen  --bom-id sfx_settle_ok
python scripts/audio/audio_pipeline.py intake --bom-id sfx_settle_ok
```
