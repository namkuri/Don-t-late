...
| 항목 | 값 |
|---|---|
| bom_id | `sfx_map_depart` |
| 종류 | SFX |
| 트리거 | (BOM 미등재) |
| 소리 | (BOM 미등재 — 생성은 규격만으로 진행) |
| 요청 길이 | 0.6s |
| dest | `Assets/Audio/SFX/sfx_map_depart.wav` |

## 창작 태그 (사람이 고치는 유일한 칸)

<!-- NOTE:BEGIN -->
cheerful two-note toy horn honk, bouncy departure jingle, bright
<!-- NOTE:END -->
...
<!-- PROMPT:BEGIN -->
```
cheerful two-note toy horn honk, bouncy departure jingle, bright. Duration about 0.6 seconds. Style: cozy cute toy-like game sound, soft wooden marimba and rounded synth plucks, playful little pitch bends, gentle and warm, light and bouncy. Single isolated sound effect, dry and close, no background music, no vocals, no long reverb tail.
```
<!-- PROMPT:END -->
...
```bash
python scripts/audio/prompt_builder.py build --bom-id sfx_map_depart
python scripts/audio/elevenlabs_client.py gen  --bom-id sfx_map_depart
python scripts/audio/audio_pipeline.py intake --bom-id sfx_map_depart
```
