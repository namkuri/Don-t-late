...
- 🔔 **R19 — 신기능 통합 플레이 판정 요청** (2026-07-28 회고 [retrospective-2026-07-28.md](retrospective-2026-07-28.md) §6-2): 남규님 1회 플레이로 다음을 판정해 주세요 — ① 도보 개척 루프 완주(빌라촌 픽업→걸어서 배치→정산→먹자 해금) ② 차 사고(치임→플래시·팝업·병원비) ③ 가방(드래그 드랍·우클릭 사용) ④ 상단 HUD 배치·크기 ⑤ 온도 스태미나 체감(폭염/한파). **성공 기준(MDA 검증 겸용)**: 마감 때문에 경로·적재를 바꾼 순간이 2회 이상 있었는가(A1 긴장감).
- 🔔 **D행 필요 — 치트 제거 시점 결정** (회고 §6-7): Y키 날씨 순환·은행 +1000 테스트 버튼 — 심사 제출 전 언제 제거할지 남규님 결정 필요.
