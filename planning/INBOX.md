...
## 🔔 R20 — 재플레이 판정 요청 (등재 2026-07-28 · S-068/S-069 수리분)

**대상**: https://namkuri.github.io/Don-t-late/ (Ctrl+F5 — S-068+S-069 신본 서빙 실측 완료)
**성공 기준 (D-070 — 관찰 가능한 문장으로 명시)**:
1. **District 프레임**: 걸어다닐 때 끊김이 체감되지 않는다 (수리 전 대비 — 가로등 그림자 8패스 제거).
2. **스태미나 게이지**: 달리면 실제로 줄어드는 것이 보인다 (렌더 수리 — 실드레인 로직은 원래 정상).
3. **하루 배송건**: 캠프 재방문 시 주문 4종이 유지되고, 다른 씬에 버린 짐이 그 자리에 남아 있으며, 정산 후에만 새 주문이 나온다.
4. **A1 긴장감 본판정** (MDA): 마감 때문에 경로/적재를 실제로 바꾼 장면이 세션당 2회 이상 있었다 → 있으면 [MDA](../docs/MDA.md) A1을 ✅로 승급.
**판정 주체**: 남규님 실플레이 (L4). 결과는 R20 피드백으로 회신 → 관제가 MDA 갱신·후속 발주 접수.
