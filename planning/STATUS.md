...
stage: "M3 — 발주 115건(완료 99) · 하루 루프 완주·전 씬 조립·날씨 7종·NPC 소셜·정산 영수증 · AAPP 실가동(RT 시트 발행 19·현존 16 — 결번 02~04는 채번 사고·재번호 이력, S-095/096 기록) · 엔딩 시퀀스 완성(S-104) · reviewer 7연속+capture-reviewer 게이트 · 잔여=아트 스왑 대기(A-002~007)·오디오 클립 2종(AU-021·022)·밸런스 튜닝"
...
- **planning/ 문서 구조 정본 (2026-07-21 규칙화 — 임의 생성물 감사 결과)**:
...
- **봉투 표준 절차 v2 (백로그 2·7 반영)**: 모든 에디터 발주 봉투에 ⓐ Bash+작은따옴표 ⓑ runInBackground
  ...
  검수 체크리스트: 게임뷰 16:9 프레이밍 · 텍스처 유무 · 종횡비 · 팔레트 4색 이탈 ·
  공간 정합(오브젝트 겹침·경계 침범·접지 — D-038, S-002에서 건물-가로등 관통을 놓친 실수→규칙)** —
  ⓕ는 검증 컷을 찍는 발주에 필수.
- **체크포인트 GC 추가 항목 (백로그 1·3·5)**: ① WebGL 회귀 빌드(커밋 묶음마다 1회 권고) ② reviewer
  서브에이전트 리허설 1회(다음 core 발주에서) ③ 발주 완료 시 eta 실측 분을 TASKS에 기입.

- **사람 처리함 = [[INBOX]]** (2026-07-20 신설): 판정 대기·결정 대기·손 작업 대기를 한 파일에 유지.
  관제는 브리핑·상태 변화 때마다 INBOX를 동기화한다 — 사람 대기 항목이 채팅에만 존재하면 규칙 위반.
- **채팅 브리핑 링크 규칙 (2026-07-21 사람 지시 — 3차 강화)**: 문서만이 아니라 **채팅 브리핑에서도**
  넘버링 ID(D-b·R7·H4·S-001…)를 언급하면 해당 md를 **클릭 가능한 마크다운 링크**로 병기한다
  (예: "R7([INBOX.md](planning/INBOX.md))"). ID 맨몸 표기는 표에서도 금지.
...
- **코드 반입 경로 규칙 (2026-07-21)**: 팀원(정수) 스크립트는 **_intake 금지 — git PR로만**
  (파일 복사는 동명 클래스 컴파일 에러 + .meta GUID 파손으로 참조 붕괴). 경로: 정수 셀프검증 3종
  → feature 브랜치 PR → 관제가 로컬 에디터에서 독립 재검증 → merge=done (HARNESS §6 판정 분리).
  _intake 화이트리스트의 "스크립트 반려"는 에셋 동봉 코드 방어용으로 유지.
