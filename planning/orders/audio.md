...
### 결과 (AU-034) · 2026-08-10 01:24 (정수 공장 · 리드 ~30분 · feature/jjs-s219-au034-slope-throw, base=main)

`Assets/Audio/SFX/sfx_throw.wav` 교체 1건. **코드 변경 0줄**(소켓 4겹 전부 실재 — 필드·재생 메서드·
발화처 2곳·`CoreSceneBuilder.LoadSfx`). 같은 파일명 덮어쓰기라 **GUID 불변**(`452fe8ad…d9a7` 전후 동일 확인).

채택 프롬프트:
```
dry cardboard box thump, one quick hit, punchy midrange, no hiss, no scratch
  + --no-anchors (토이톤 앵커 제거) · 0.5s · seed 662102799 · sha1 c1264adaa13c
```
...
| 라운드 | 태그 방향 | take | 판정 |
|---|---|---|---|
| ① | `crisp marimba tok`(앵커 유지) | 9 | 기각 — "삑 소리, 던지는 것 같지 않다" |
| ② | `short scuff`(`--no-anchors`) | 6 | 기각 — "그나마 던지는 소리 같은데 췩 같이 거슬린다" |
| ③ | `thump` + `no hiss, no scratch` | 6 | **m26 채택** |

- ①의 원인은 **토이톤 앵커가 음정을 만든다**는 것. 던지는 순간은 음정이 없다 —
  마림바·신스 플럭 앵커는 hop·plop엔 맞아도 임팩트엔 구조적으로 안 맞는다(AU-018 ③ 기록과 같은 결).
- ②의 "췩"은 2겹: 태그의 `scuff`(마찰=고역) + 원본이 -22~-33dB로 작아 게인 20~30dB에 히스가 동반 증폭.
- ③의 m26만 **raw peak 0.0dB** — 증폭이 불필요해 히스 원천이 없다. 이게 채택의 실질 근거다.
- **후공정에 1-pole 저역통과 3kHz를 이번 건 한정으로 추가**: 채택본에 남은 고역을 Director가 지적해
  5k/3k 두 벌 발신 → 3k 지목. 순서 = LPF → 트림 → 단일게인 → 8ms 페이드 → 모노.
  실측 **0.175s · 모노 44.1kHz · peak -3.9dB · rms -14.0dB**(RMS 한계) · Unity 재임포트 후
  `len=0.175s ch=1 loadType=DecompressOnLoad` 확인.
- 생성 실패율: 21 take 중 **무음 7건**. 앵커 유무와 무관 — 임팩트 계열은 톤 계열보다 무음 빈도가 높다.
- **BOM 등재는 불요로 정정**: 발주서에 "BOM §8 트리거·소리 칸 등재"를 적었으나, §15.1(2026-07-29)이
  `sfx_throw`를 포함한 미등재 23종을 "개별 행 분해 불요 — 트리거 매핑은 시스템 발주 결과가 정본"으로
  이미 처분했다. BOM은 동결 문서라 불필요한 행 추가는 오히려 규율 위반이다.
- 프롬프트 문서 세대 이력에 ①②라운드 조립분(gen 8·9)이 누락돼 있다 — 작업 중 `prompt_builder`가
  Windows 파일 잠금(Errno 22)으로 한 번 실패해 문서를 git 복구했고, 그 뒤 재조립분이 gen 8로 이어졌다.
  프롬프트 정본은 이 결과 블록과 CREDITS의 채택 태그다.
- 콘솔 에러 0 · 워닝 0. **Play 실청취 검증은 남규님이 직접 진행 예정**(효과음 판정은 사람 귀).
