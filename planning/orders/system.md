...
## S-053 · 발주 2026-07-25 (Director 직접 지시 — 대사 효과음 8비트 블립 2종 적용)

요구 (Director 원문): "`rapid_stepped_8-bit__#2`·`#3` 2개의 곡을 가지고 대사 효과음에 적용할 수 있을까? (기존 곡은 안지우고 보관)".

수용기준: 2종 블립 반입·대사 효과음 적용 · 기존 블립 보관 · 라이선스 등재 · 셀프검증.

### 결과 · 2026-07-25 (리드 ~25분)
- 검증: 2파일 2ch/16bit/48kHz/**0.48s** 블립. 출처 = ElevenLabs SFX(Director 확인 — 기존 SFX와 동일 계정·권리).
- 반입: `sfx_dialogue_blip_1.wav`(원 #2)·`sfx_dialogue_blip_2.wav`(원 #3) → `Assets/Audio/SFX/`. 임포터 자동(모노·DecompressOnLoad·Vorbis, 콘솔 0).
- 코드: `DialogueView`에 `_blipClips[]` 풀 신설 + `PlayBlip`이 **문자마다 랜덤 선택**(비면 `_blipClip` 폴백, 피치 0.95~1.05 변주 유지). `CoreSceneBuilder`가 변주 2종 로드·배선(리플렉션 주입).
- 기존 `sfx_dialogue_blip.wav`는 **삭제 없이 `_blipClip` 폴백으로 보관**(Director 지시).
- 검증: 컴파일 ○ · 콘솔 0 ○ · Core 재빌드 후 저장 YAML에 변주 2 guid 실존(fileID:0 유실 함정 회피) ○ · Play 대사 구동 시 블립 소스 pitch 변경(PlayBlip 발화 증거)·텍스트 렌더 ○. **음질 청취는 Director 사람 판정**.
- 커밋 경계: 코드·SFX asset·문서만. Core.unity 씬 본문은 커밋 제외(빌더 정본·재현).
