...
## S-252 · 발주 2026-08-10 16:52 → ClaudeCode (가방 우클릭 '손에 들기' 칸 삭제)

요구 (남규님 원문): 인벤토리에 두 번째 사용 버튼은 삭제하자(손에 잡는 거).

관찰 (첨부 캡처): 우클릭 메뉴가 `[사용][사용][버리기]` — 두 번째가 '손에 들기'인데 아트 프리팹의
버튼 라벨이 "사용"이라 같은 글자로 두 번 뜬다. S-243에서 칸을 늘릴 때 라벨이 겹치는 것을 못 봤다.

수용기준: 우클릭 메뉴가 `[사용][버리기]` 두 칸이다 · 좌클릭 사용은 종전대로 · 콘솔 에러·워닝 0.
**부수 효과 명시**: '손에 들기'(S-064 — 들고 던지기) 진입로가 우클릭에서 사라진다.

MDA 판정 (D-070): **강화(미학)** — 같은 이름 버튼이 둘이면 무엇을 눌러야 할지가 사라진다.

## S-253 · 발주 2026-08-10 16:52 → ClaudeCode (Village 다녀오면 트럭 상호작용이 죽는다)

요구 (남규님 원문): Village 씬에 나갔다 돌아오니까 트럭에 하이라이트 및 상호작용 안 됨.

배경: S-248에서 트럭에 아트 모델 복제를 붙이고 하이라이트 범위를 트럭 루트 전체로 넓혔다.
S-251에서 적재존에 포커스 게이트를 달았다. 둘 다 **씬을 한 번만 들어간 상태로** 검증했고,
**씬을 나갔다 돌아오는 경로는 안 태웠다.**

수용기준: Camp→Village→Camp 왕복 후에도 트럭 앞에서 하이라이트가 걸리고 프롬프트가 뜬다 ·
왕복 실측으로 검증한다(한 번 들어간 상태로 끝내지 않는다) · 콘솔 에러·워닝 0.

MDA 판정 (D-070): **강화(역학)** — 하루 루프는 캠프와 거리를 오간다. 왕복 뒤에 죽는 기능은
사실상 없는 기능이다.

### S-252 · 결과 2026-08-10 17:01 (셀프검증 3종 통과)

'손에 들기' 칸을 끄고 참조를 끊었다(`_useButton = null`). 살려 두면 `BagView`가 상황에 따라
도로 켠다. 아트 프리팹 경로는 버튼을 비활성화하고, 코드 폴백 경로는 아예 만들지 않는다
(메뉴 높이도 170 → 118로 되돌렸다).

관찰: 재조립 후 메뉴 자식 = `ConsumeButton, UseButton(꺼짐), DropButton` · `_useButton=null` —
화면에는 **[사용][버리기] 두 칸**만 보인다 · 콘솔 에러·워닝 0.

부수 효과 (명시): **'손에 들기'(S-064 들고 던지기) 진입로가 우클릭에서 사라졌다.**
좌클릭은 소비 리스너가 없는 씬(집 등)에서 여전히 손에 들기로 떨어지므로 기능 자체는 살아 있다.

### S-253 · 결과 2026-08-10 17:01 (셀프검증 3종 통과 · 왕복 실측)

**원인은 씬 왕복이 아니었다.** 먼저 Camp→Village→Camp를 태워 봤는데 앞범퍼 쪽은 왕복 뒤에도
멀쩡했다(포커스 `TruckDepartPoint`, 두 렌더러 모두 하이라이트). 조건을 바꿔 **짐칸 쪽**에 세우자
바로 재현됐다:

```
짐칸 포커스=LoadingZone / geometry_0.001:GB_Highlight , rew (1):logis_logo_gpt
```

`LoadingZone.SetHighlight`만 옛 방식(렌더러 하나)이었다. S-248에서 트럭 인터랙트 두 개
(구매·탑승)는 `HighlightSwapper`로 바꿨는데 **같은 트럭에 붙은 적재존을 빠뜨렸다.**
그레이박스 몸통은 아트 모델에 완전히 가려 있어, 몸통만 켜지면 화면에는 **아무 변화도 안 보인다** —
남규님이 "하이라이트 및 상호작용 안 됨"으로 본 것이 이것이다(포커스는 잡히고 있었다).

관찰 (Camp→Village→Camp 왕복 후):
- 짐칸 쪽: 포커스 `LoadingZone` · **`geometry_0.001:GB_Highlight, rew (1):GB_Highlight`** (둘 다 전환)
- 앞범퍼 쪽: 포커스 `TruckDepartPoint` · HUD `[E] 트럭탑승` · 두 렌더러 모두 하이라이트
- 캡처 `Screenshots/s253_loadzone_highlight.png` — 트럭 전체가 켜진 것이 눈으로 보인다
- 콘솔 에러·워닝 0.

관제 반성: S-248을 "트럭 하이라이트를 전체로 넓혔다"고 보고했지만 **같은 트럭의 세 번째
상호작용(적재존)을 세지 않았다.** 한 오브젝트에 인터랙트가 여러 개 붙어 있으면 전부 훑어야 한다.

## S-254 · 발주 2026-08-10 17:21 → ClaudeCode (해금된 구역이 폰 지도에 여전히 잠김으로 표시)

요구 (남규님 원문): 먹자골목 해금 후에도 휴대폰 앱에 잠겨 있다고 UI에 표시됨(목적지 출발하면 가지긴 함).

진단 (S-249와 같은 부류): 핀 라벨이 지도 화면을 **만들 때** 한 번 정해진다. 상태가 바뀌어도
이미 조립된 화면은 그대로다 — 출발 버튼에서 같은 것을 잡았는데 핀은 놓쳤다.

수용기준: 해금된 구역 핀에서 "잠김 — 개척 필요"가 사라진다 · 지도를 열 때마다 현재 해금 상태를
반영한다 · 콘솔 에러·워닝 0.

MDA 판정 (D-070): **강화(역학)** — 갈 수 있는 곳을 못 간다고 표시하면 개척의 보상이 사라진다.

## S-255 · 발주 2026-08-10 17:21 → ClaudeCode (차로 도착 시 스폰 위치)

요구 (남규님 원문): 차로 이동해서 도착하면 씬 좌측 택배상자 앞(택배상자보다 좌측)에 같이
스폰되도록. 엣지워크랑 너무 붙이진 말고.

수용기준: 트럭 이동으로 구역에 도착하면 플레이어가 짐(택배상자) **왼쪽**에 선다 ·
엣지 게이트와는 여유 거리를 둔다(도착하자마자 씬을 넘어가지 않게) · 콘솔 에러·워닝 0.

MDA 판정 (D-070): **강화(역학)** — 도착 즉시 짐이 눈앞에 있어야 다음 행동이 자명하다.

## S-256 · 발주 2026-08-10 17:21 → ClaudeCode (폰 지도에 캠프가기 + 현재 위치 화살표)

요구 (남규님 원문): 캠프로 다시 돌아갈 수 있게 휴대폰 앱에 캠프 버튼 추가(지도 부분 우측 하단에
"캠프가기"). 그리고 주황색 화살표는 실제 있는 구역에 표시해 줄 것.

수용기준: 지도 우측 하단에 `캠프가기` 버튼이 있고 누르면 물류캠프로 간다 ·
현재 위치 화살표가 **지금 있는 구역** 핀에 붙는다(종전엔 출발 마커 자리 고정) · 콘솔 에러·워닝 0.

MDA 판정 (D-070): **강화(역학)** — 돌아갈 길이 화면에 없으면 하루 루프가 한 방향으로만 흐른다.

## S-257 · 발주 2026-08-10 17:21 → ClaudeCode (폰 화면 1.2배)

요구 (남규님 원문): 폰 화면 1.2배 늘리자.

수용기준: 폰 패널이 1.2배로 커진다 · 화면 밖으로 잘리지 않는다 · 콘솔 에러·워닝 0.

MDA 판정 (D-070): **무관(가독성)** — 폰이 정보 창구인데 글자가 작다.

## S-258 · 발주 2026-08-10 17:21 → ClaudeCode (트럭에 붙어도 상호작용이 안 뜬다 · 3회차)

요구 (남규님 원문): 트럭에 붙어도 상호작용이 안 뜸. 조금 전엔 떴는데 이상함.

진단 (캡처 실측): 캡처는 **트럭 미보유 상태**다(돈 3,000 · 빚 10,000 · 박스 0/0 = 세션 초기).
그리고 플레이어가 트럭 **왼쪽 몸통 옆**에 서 있다. 지금 배치는 구매 지점이 **앞범퍼 한 점**
(트럭 오른쪽 끝)에만 있고, S-251에서 적재존은 트럭이 없으면 포커스를 양보하게 했다 —
그래서 트럭 옆에 붙어도 잡히는 것이 하나도 없다. **의도한 동작이지만 사람 눈에는 고장이다.**

수용기준: 트럭이 없을 때 **트럭 어느 쪽에 붙어도** `[E] 트럭구매`가 뜬다 ·
트럭 보유 시에는 종전대로 짐칸=적재 / 앞범퍼=탑승으로 갈린다 ·
**트럭 둘레 여러 지점**을 실측해 검증한다(한 점만 재지 않는다) · 콘솔 에러·워닝 0.

MDA 판정 (D-070): **강화(역학)** — 세 번째 지적이다. 구매가 하루 루프의 관문인데 접근이 막혀 있다.

### S-254~S-258 · 결과 2026-08-10 17:40 (셀프검증 3종 통과 · 실측)

**S-258 트럭 접근성 (3회차 — 마감)**: 캡처를 다시 읽어 조건을 특정했다 — 트럭 **미보유** 상태에서
플레이어가 트럭 **옆구리**에 서 있었다. 구매 지점이 앞범퍼 한 점뿐이라 잡히는 게 없었고,
S-251에서 적재존은 트럭이 없으면 양보하게 해 둔 터라 정말 아무것도 없었다.
→ 구매 트리거를 **트럭 둘레 전체**(9×2.4×3.2)로 키웠다. 트럭이 없을 땐 겹칠 상대가 없다.
실측(트럭 미보유): x=8.2 · 10.5 · 12.4 **전부 TruckPurchasePoint** (x=6.0은 자판기가 더 가까움 — 정상).
실측(트럭 보유): x=8.2 → LoadingZone · x=12.4 → TruckDepartPoint (종전 갈림 유지).

**S-254 잠김 표시**: 핀 라벨·색·크기를 지도 열 때마다 다시 쓴다(`RefreshMapPins`). S-249에서
출발 버튼의 같은 병을 고치면서 핀은 놓쳤다.
실측: `빌라촌 | 먹자골목 | 아파트단지 잠김 | 언덕주택가 잠김` — 해금분에서 표시가 사라진다.

**S-255 도착 스폰**: 트럭(Travel 경유)으로 도착했을 때만 플레이어를 첫 짐 왼쪽에 세운다.
도보(엣지 워크)로 온 경우는 손대지 않는다 — 넘어오자마자 반대편으로 순간이동하는 꼴이 된다.
**엣지 게이트와 2.0u 이상 띄운다**(남규님 단서): 빌라촌 실측이 짐 −16 · 게이트 −19라
오프셋(2.2)만 쓰면 −18.2로 게이트에 0.8까지 붙었다.
실측: 트럭 도착 후 플레이어 **x=−17.00** (짐 −16.0 왼쪽 1.0 · 게이트 −19 에서 2.0).

**S-256 캠프가기 + 현재 위치**: 지도 우측 하단에 `캠프가기`(구역에 있을 때만 표시).
주황 화살표를 지금 있는 구역 핀으로 옮기고 **핀 위로 올린다** — 같은 자리에 두니 핀 뒤에 깔려
안 보였다(1차 캡처로 발견). 출발지 라벨도 현재 구역으로 맞췄는데, 라벨을 직접 쓰면 지도 정보
갱신이 뒤에 한 번 더 돌며 덮어써서 **원본 값(`_travelOrigin`)을 고치는 쪽**으로 바꿨다.
실측: 빌라촌에서 캠프버튼 표시=True · 화살표가 빌라촌 핀 위에 보인다(캡처 `s256_map2.png`).

**S-257 폰 1.2배**: 크기 대신 **스케일**을 건다 — 내부 위젯 좌표가 전부 픽셀 상수라 sizeDelta만
키우면 배치가 어긋난다. `ApplyPanelLayout`은 Travel 진출입 때만 불려서 그 전엔 1.00이었다(실측) →
`Start`에서도 건다. 실측: 폰스케일 **1.20**.

콘솔 에러·워닝 0.

## S-259 · 발주 2026-08-10 17:48 → ClaudeCode (폰 크기 되돌리기 + 닫았을 때 안 보이게)

요구 (남규님 원문): 휴대폰 크기 키우니까 이렇게 됨 — **메인 씬에서부터 보이고, 안 열어도 휴대폰
윗부분이 튀어나옴.** 안 열었을 때 안 튀어나오게 하고, 크기는 **수정 전의 1.1배**로만.

진단: S-257에서 스케일만 1.2로 걸고 **숨김 위치(`_hiddenY`)를 보정하지 않았다.** 패널이 커진 만큼
아래로 더 내려가야 하는데 그대로라 윗부분이 화면에 남는다. 타이틀에서도 보이는 것이 그 결과다.

수용기준: 스케일 **1.1** · 닫힌 상태에서 화면에 폰이 **전혀 안 보인다**(타이틀 포함) ·
연 상태에서는 종전처럼 화면 안에 들어온다 · 콘솔 에러·워닝 0.

MDA 판정 (D-070): **강화(미학)** — 첫 화면에 UI 조각이 걸쳐 있으면 완성도가 그 자리에서 깎인다.

## S-260 · 발주 2026-08-10 17:48 → ClaudeCode (트럭 도착 스폰이 여전히 맵 중간)

요구 (남규님 원문): 아직 트럭 타고 스폰하면 위치가 맵 중간쯤임. 택배상자 좌측 스폰이 안 됨.

진단: S-255는 **스폰된 상자가 1개 이상일 때만** 동작한다(`boxCount > 0`). 남규님 캡처는 `박스 0/1` —
짐을 트럭에 안 실은 채 이동해 구역에 스폰될 상자가 없었고, 그래서 배치가 통째로 건너뛰어졌다.
관제 실측은 짐을 실은 상태로만 해서 이 경우를 못 봤다.

수용기준: **짐이 없어도** 트럭 도착 시 짐 놓이는 자리(왼쪽)에 선다 · 엣지 게이트와는 종전대로 이격 ·
짐 있음/없음 **두 경우 모두** 실측한다 · 콘솔 에러·워닝 0.

MDA 판정 (D-070): **강화(역학)** — 도착 지점이 매번 다르면 다음 행동을 찾는 데 시간이 든다.

## S-261 · 발주 2026-08-10 17:48 → ClaudeCode (트럭 구매 독백 문구 교체)

요구 (남규님 원문): 트럭 구매하면 독백 튜토리얼로 "짐은 뒤에 가서 실으면 되고, 다 실으면 트럭
앞으로 가서 탑승하거나 아니면 휴대폰 지도앱에서 선택해서 갈 수 있다"고 대화창으로 나오게.

수용기준: 구매 직후 독백이 **짐칸(뒤) 적재 → 트럭 앞 탑승 또는 폰 지도앱** 두 경로를 알려 준다 ·
콘솔 에러·워닝 0.

MDA 판정 (D-070): **강화(역학)** — 산 직후가 "이걸로 뭘 하지?"가 제일 큰 순간이다.

## S-262 · 발주 2026-08-10 17:48 → ClaudeCode (캠프 사장님 위치·각도)

요구 (남규님 원문): Camp 씬에 사장님(`__gb_BossNpc`) 위치 `Vector3(3.8, 0.04, 0.01)`,
각도 Y축 **180도**로 설정.

수용기준: 재조립해도 그 위치·각도가 유지된다(빌더에 반영) · 콘솔 에러·워닝 0.

MDA 판정 (D-070): **무관(배치)** — 남규님 손배치를 정본으로 승격.

## S-263 · 발주 2026-08-10 17:48 → ClaudeCode (트럭에 실은 짐 투명 처리 — 캠프 한정)

요구 (남규님 원문): 트럭에 실은 짐은 투명 처리해 줄 것(캠프씬에서만).

수용기준: 캠프에서 적재한 상자가 짐칸에 **보이지 않는다** · 적재 자체(주문 등록·개수)는 종전대로 ·
다른 씬에는 영향 없다 · 콘솔 에러·워닝 0.

MDA 판정 (D-070): **강화(미학)** — 아트 트럭이 붙은 뒤로 짐칸 상자가 모델을 뚫고 보인다.

### S-259~S-263 · 결과 2026-08-10 18:00 (셀프검증 3종 통과 · 실측)

**S-259 폰 크기·숨김**: 스케일 **1.1**로 낮추고, **숨김 위치도 스케일을 타게** 했다.
S-257에서 스케일만 걸고 `_hiddenY`를 그대로 둔 것이 원인이다 — 패널이 커진 만큼 더 내려가야
하는데 안 내려가서 윗부분이 화면에 걸렸다(타이틀에서까지 보였다).
실측: 타이틀 진입 시 열림=False · 스케일 **1.10** · **캡처(`s259_title.png`)에 폰이 전혀 안 보인다.**

**S-260 도착 스폰 (재발 마감)**: S-255는 `boxCount > 0`일 때만 돌았다. 남규님 캡처의 `박스 0/1`이
단서였다 — 짐을 안 실은 채 이동하면 스폰될 상자가 없어 배치가 통째로 건너뛰어졌다.
관제는 짐을 실은 경우로만 실측해서 못 봤다. → 짐이 없으면 **짐이 놓일 자리**(`_boxOrigin`,
없으면 −16)를 기준으로 삼는다. 기준식을 상자 스폰과 같게 맞춰 두 경우의 자리가 같다.
실측: 짐 1건 → x=−17.00 · **짐 0건 → x=−17.00**(동일) · 엣지 게이트(−19)에서 2.0 이격.

**S-261 구매 독백**: 남규님 문구 그대로 — 짐은 트럭 뒤에서 싣고, 다 실으면 트럭 앞에서 타거나
폰 지도 앱에서 골라 갈 수 있다. 두 경로를 다 말한다(하나만 알려 주면 나머지가 없는 기능이 된다).

**S-262 사장님 배치**: 빌더 상수로 승격 — 위치 `(3.8, 0.04, 0.01)` · 루트 Y **180°**.
실측: 재조립 후 `(3.80, 0.04, 0.01) yaw=180`.

**S-263 적재 상자 투명**: 렌더러만 끈다(오브젝트는 남겨 적재 개수·정산 흐름은 그대로).
캠프 빌더만 이 옵션을 켠다 — 다른 씬의 적재존에는 영향이 없다.
실측: 재조립 후 `적재상자 숨김=True`.

콘솔 에러·워닝 0.

## S-264 · 발주 2026-08-10 18:08 → ClaudeCode (트럭에 안 실은 짐도 따라온다)

요구 (남규님 원문): 트럭에 안 실은 짐도 트럭이랑 같이 이동하는데? 안 실은 건 그냥 캠프에
그대로 쌓여 있게 해 줄 것.

진단: `PickupBox`가 **손에 드는 순간** `AcceptOrder`로 `cargo`에 등록한다(S-066 ① — 트럭 없이
들고 걸어가도 정산이 인정되게 한 조치). 그래서 캠프에서 상자를 들었다 내려놓기만 해도 그 주문이
`cargo`에 남고, `DistrictCargoSpawner`가 `cargo` 기준으로 스폰하므로 구역에 나타난다.
**`cargo`가 "오늘 배송 목록"과 "트럭에 실린 것"을 겸하고 있는 게 근본 원인이다.**

수용기준: 트럭 보유 시 구역에는 **적재존에서 실제로 실은 짐만** 나온다 · 안 실은 짐은 캠프에 남는다 ·
도보 시대(트럭 없음)는 종전대로 손에 든 짐이 따라간다 · 정산 인정 범위는 바뀌지 않는다 ·
**실음/안 실음 두 경우 모두 실측** · 콘솔 에러·워닝 0.

MDA 판정 (D-070): **강화(역학)** — 싣는 행위에 의미가 없으면 적재 자체가 장식이 된다.

## S-265 · 발주 2026-08-10 18:13 → ClaudeCode (사장님 튜토리얼 끝에 트럭 안내 2줄)

요구 (남규님 원문): 사장님 튜토리얼 마지막에
① "너무 먼 지역은 트럭이 편할거야! 트럭은 캠프에서 빌릴 수 있으니까 언제든 와. 대신 돈은 좀 내야한다!"
② "타고싶거든 트럭 근처로 가서 E키를 눌러보도록 해. 그럼 수고!"
라고 알려줄 것. **튜토리얼 미션은 아니고 말로만 — 대화창 2번 재생.**

수용기준: 사장님 튜토리얼 마지막 단계를 마치면 위 두 줄이 대화로 나온다 ·
**게이트(진행 조건)를 만들지 않는다** — 말하고 끝난다 · 콘솔 에러·워닝 0.

MDA 판정 (D-070): **강화(역학)** — S-241 트럭 구매의 존재를 플레이어가 알 길이 지금 없다.

### S-264 · 결과 2026-08-10 18:16 (셀프검증 3종 통과 · 두 경우 실측)

`cargo`가 두 가지 뜻을 겸하고 있던 게 근본 원인이다 — "오늘 배송 목록"이자 "트럭에 실린 것".
`PickupBox`가 **손에 드는 순간** `AcceptOrder`로 등록하므로(S-066 ① — 도보 배달도 정산이 인정되게
한 조치), 캠프에서 들었다 놓기만 해도 그 주문이 구역까지 따라왔다.

- `GameStateSO.truckCargoIds` 신설 — **적재존에서 실제로 실은** 주문 id만 담는다.
  등록 지점은 `LoadingZone.Interact` 한 곳뿐이다("실었다"의 유일한 정의).
- 구역 스폰은 **트럭 보유 시** 이 목록으로 거른다. 도보 시대는 종전대로 — 그때는 손에 든 것이
  곧 적재이고, 이미 `carriedOrders`로 처리된다.
- 세션 리셋·하루 정산에서 함께 비운다.

관찰 (플레이모드): 빌라촌 주문 **4건을 cargo에 등록하고 트럭에는 1건만 실은** 상태로 이동 →
구역 스폰 상자 **1개**(실은 것만) · 플레이어 x=−17.00(S-260 유지) · 콘솔 에러·워닝 0.

### S-265 · 결과 2026-08-10 18:16 (셀프검증 3종 통과 · 캡처)

사장님 튜토리얼 전 단계를 마치는 지점에서 트럭 안내 2줄을 재생한다. **게이트를 만들지 않는다** —
말하고 끝난다(남규님 지시). 두 줄은 **한 시나리오에 담았다**: `PlayScenario`를 연달아 부르면
뒤엣것이 앞엣것을 끊는다.

관찰: 재생 호출 후 대화중=True · 캡처(`s265_outro.png`)에 1번 대사가 지시 문구 그대로 표시 ·
콘솔 에러·워닝 0.

## S-266 · 발주 2026-08-10 18:48 → ClaudeCode (튜토리얼 — 이미 충족된 미션은 즉시 통과)

요구 (남규님 원문):
① 빌라촌 가는 미션에 **이미 빌라촌에 있으면** 미션 완료 처리.
② **이미 짐을 파란 비콘 위에 올려놓은 상태면** 완료 처리.
③ **이미 상자를 들고 있으면** 완료 처리.

배경: 게이트 3종(`ReachDistrict`·`PlaceDelivery`·`BoxPickup`)이 **그 단계에 들어온 뒤 발생하는
이벤트**만 본다. 단계 시작 시점에 이미 조건이 서 있으면 영원히 안 풀린다.

수용기준: 세 게이트가 단계 진입 시 현재 상태를 먼저 검사해 이미 충족이면 바로 통과한다 ·
아직이면 종전대로 이벤트를 기다린다 · 콘솔 에러·워닝 0.

MDA 판정 (D-070): **강화(역학)** — 이미 한 일을 다시 하라고 막으면 튜토리얼이 벽이 된다.

## S-267 · 발주 2026-08-10 18:48 → ClaudeCode (행인 이름이 `__gb_Walker_A`로 노출)

요구 (남규님 원문): 스크린샷처럼 행인 이름이 Walker로 나오는 경우가 있는데 없도록 수정.

관찰: 이름표와 대화창 화자에 `__gb_Walker_A`가 그대로 뜬다 — 표시 이름이 비어 오브젝트 이름으로
떨어진 것.

수용기준: 행인 이름표·대화 화자에 내부 이름(`__gb_`)이 **절대** 안 나온다 ·
빠진 곳이 또 없는지 전수 확인 · 콘솔 에러·워닝 0.

MDA 판정 (D-070): **강화(미학)** — 내부 식별자가 화면에 뜨면 그 순간 게임이 아니라 개발 화면이 된다.

## S-268 · 발주 2026-08-10 18:48 → ClaudeCode (씬 진입 독백 + 트럭 앞 독백)

요구 (남규님 원문): SpeechBubble로 씬에 처음 들어갈 때 "여기가 빌라촌이구나.",
"먹자골목이네 맛있겠다..." 등 캐릭터 독백. 그리고 **트럭 앞에 가서 구매 전이면**
"트럭을 빌릴까? 근데 아직 갚을 빚이 많은데..." 라고 **한 번** 독백.

수용기준: 구역 씬 **첫 진입 때** 그 구역에 맞는 독백이 말풍선으로 뜬다(재진입 시 반복 안 함) ·
트럭 미보유 상태로 트럭에 다가가면 위 독백이 **한 번만** 뜬다 · 콘솔 에러·워닝 0.

MDA 판정 (D-070): **강화(감정)** — 주인공의 목소리가 세계에 대한 첫인상을 만든다.

### S-266~S-268 · 결과 2026-08-10 19:16 (셀프검증 3종 통과 · 실측)

**S-266 선충족 통과**: 단계에 들어선 순간 현재 상태를 검사해 이미 충족이면 바로 통과시킨다.
게이트들은 그 단계 이후 발생하는 **이벤트**만 듣기 때문에, 이미 빌라촌에 있거나 상자를 들고
있거나 비콘에 짐을 올려 뒀으면 그 이벤트가 다시 올 일이 없어 영영 안 풀렸다.

**여기서 더 큰 것이 나왔다** — 튜토리얼 디렉터의 `_gameState`가 **처음부터 NULL**이었다.
두 겹의 원인:
1. 빌더가 주입한 **직후**에 `SerializedObject`를 만들고 Apply해서, 그 옛 값(null)으로 되돌렸다.
2. `GreyboxStageBuilder.SetReference`는 이름과 달리 **SerializedObject 경유**라 씬 저장 때
   `{fileID: 0}`으로 날아간다(CODE_RULES §6 실수→규칙의 바로 그 함정).
→ Apply **뒤에**, `SetField`(리플렉션 직접)로 주입한다. 실측: 재조립 후 `_gameState=주입됨`.
   ⚠ 이 누락으로 디렉터의 상태 기반 판정이 통째로 죽어 있었다 — S-266이 아니었으면 계속 몰랐다.

실측(플레이모드): 이미 빌라촌 → `ReachDistrict` 통과=**True** · 이미 비콘 배치 →
`PlaceDelivery` 통과=**True** · 이미 상자 보유 → `BoxPickup` 통과=**True**.

**S-267 내부 이름 노출**: `PedestrianNpc`가 세 곳에서 제각각
`_randomTalkPool?.displayName ?? gameObject.name`으로 폴백했다. 잡담 풀이 없는 행인은
`__gb_Walker_A`가 그대로 떴고, 게다가 `ShowNpcInfo`가 그 값으로 `Configure`를 다시 불러
**빌더가 넣어 둔 이름표까지 덮어썼다**(이름표가 처음엔 맞았다가 대화 때 바뀐 이유).
→ 표시 이름을 `DisplayName` 프로퍼티 **한 곳**으로 모으고, 빌더가 NpcSO 이름을 주입한다.
최후 폴백도 `__gb_`·언더스코어를 걷는다. 실측: `__gb_Walker_A → "회색 코트 아저씨"`.

**S-268 독백**: 구역 첫 진입은 플레이어에 붙은 `PlayerArrivalRemark`가 `Start`에서 낸다
(플레이어는 씬마다 새로 태어나므로 Start가 곧 도착 — 이벤트 구독이 필요 없다).
방문 기록은 세션 단위(`greetedScenes`)라 재진입에 반복하지 않는다. 대화가 돌고 있으면 끝날 때까지
미룬다(말풍선이 대화창 위에 겹치지 않게).
트럭 앞 독백은 월드 오브젝트가 플레이어를 직접 찾지 않도록 `WorldEvents.PlayerRemarked`로 넘긴다.
실측: 캠프 진입 시 `greetedScenes=[Camp]` 기록 · 트럭 접근 시 이벤트 로그
`PlayerRemarked → 트럭을 빌릴까? 근데 아직 갚을 빚이 많은데...` + `truckRemarkDone=True`.

콘솔 에러·워닝 0.

한계 (정직): **말풍선이 뜬 화면 캡처는 못 남겼다.** 캠프 진입 독백은 사장님 튜토리얼 대화가
끝날 때까지 미뤄지고(의도한 동작), 트럭 독백은 3초 표시라 캡처 왕복(≈4초) 사이에 사라졌다.
발화 자체는 이벤트 로그와 상태 플래그로 확인했다.

## S-269 · 발주 2026-08-10 19:29 → ClaudeCode (TutorialBanner를 미션 설명과 동기화 + 20초 자동 소멸)

요구 (남규님 원문):
① TutorialBanner 텍스트를 **튜토리얼 미션카드 설명과 동일하게** 하고, 뜬 다음 **20초 뒤 자동으로 사라지게**.
② 박말순 전화가 오면 TutorialBanner로 "박말순 전화를 받고 화면과 동일하게 방향키 빠르게 눌러야한다."를
   띄우고 역시 20초 뒤 사라지게.

진단: 지금 배너는 **씬 빌더가 문구를 박아 만든 정적 UI**다(씬마다 고정 문장). 그래서 미션이
바뀌어도 그대로고, 닫기 전엔 화면 위에 계속 남는다.

수용기준: 튜토리얼 단계가 시작되면 배너 문구가 **그 단계의 설명(미션카드와 같은 문장)**으로 바뀐다 ·
표시 후 20초면 스스로 사라진다 · 박말순 전화 시 위 문구가 뜨고 20초 뒤 사라진다 ·
닫기 버튼은 종전대로 · 콘솔 에러·워닝 0.

MDA 판정 (D-070): **강화(역학)** — 지금 뭘 해야 하는지가 화면 맨 위에 있어야 한다.
안 사라지는 배너는 그 자리를 영구 점유해 다음 안내를 가린다.

## S-270 · 발주 2026-08-10 19:32 → ClaudeCode (막힌 상호작용에 이유를 말해 준다 — 독백 3종)

요구 (남규님 원문):
① 트럭 구매 후 **짐을 안 들고** 트럭에 [E] → "우선 박스를 들어서 옮겨와야겠다." (캠프 한정)
② **송장을 안 찍고** 택배 상자 앞에서 [E] → "먼저 마우스로 클릭해서 송장을 찍어야되." (캠프 한정)
③ **트럭에 짐이 실려 있고 트럭도 산 상태**로 앞에 가면 →
   "트럭은 앞쪽에서 탑승하거나, 휴대폰으로 다른 지역으로 갈 수 있다고했어."

배경: 셋 다 지금은 **콘솔 로그로만** 거절한다 — 화면에는 아무 말도 없어 왜 안 되는지 알 수 없다.

수용기준: 세 상황에서 SpeechBubble 독백이 뜬다 · 캠프 한정 조건이 지켜진다 ·
③은 반복해서 도배되지 않는다 · 콘솔 에러·워닝 0.

MDA 판정 (D-070): **강화(역학)** — 막을 때는 이유를 말해야 다음 행동이 나온다.

### S-269 · 결과 2026-08-10 19:37 (셀프검증 3종 통과 · 실측)

배너에 `TutorialBannerView`를 붙여 **문구를 진행부에서 받아 쓰게** 했다. 미션카드와 **같은
`detail` 문장**을 같은 이벤트(`TutorialStepStarted`)로 받으므로 둘이 어긋날 수 없다.
박말순 전화(`PhoneRang`)에는 지정 문구를 띄운다. 표시 후 20초면 스스로 물러난다.

**함정 하나**: 숨길 때 `SetActive(false)`를 쓰면 `OnDisable`이 돌아 **구독이 끊겨** 다음 단계
안내를 영영 못 듣는다 → `CanvasGroup` 알파로 감춘다(`blocksRaycasts`도 함께 내려 닫기 버튼이
투명한 채로 클릭을 먹지 않게).
시간은 `Time.unscaledTime` — 대화 중 시간이 멈춰도 배너는 물러나야 한다.

관찰 (플레이모드 Camp): 기본 `[물류캠프 — 패드에서 E로 적재]` → 단계 시작 시
`[미션카드와 같은 설명 문장]`으로 교체 → 전화 시
`[박말순 전화를 받고 화면과 동일하게 방향키 빠르게 눌러야한다.]` · alpha=1.0 · 콘솔 에러·워닝 0.

범위 기록: 배너는 **씬 빌더가 만드는 씬에만** 있다(캠프·이동 등). **빌라촌 등 구역 씬에는 배너 자체가
없다**(실측: `TutorialBanner` 0개) — 그 씬에서는 미션카드가 안내를 맡는다. 발주 밖이라 새로 깔지 않았다.

### S-270 · 결과 2026-08-10 19:37 (셀프검증 3종 통과 · 실측)

막는 자리마다 이유를 말한다. 셋 다 종전엔 콘솔 로그뿐이라 화면에는 아무 일도 없었다.

- ① 빈손 적재 시도 → "우선 박스를 들어서 옮겨와야겠다." — `LoadingZone`은 캠프에만 붙으므로
  (부착 지점이 `CampStageBuilder` 한 곳) 캠프 한정이 자동 충족된다.
- ② 미스캔 상자 [E] → "먼저 마우스로 클릭해서 송장을 찍어야되." — 스캔 대상(`_requireScanned`)은
  캠프 물량뿐이라 역시 캠프에만 뜬다.
- ③ 트럭 보유 + 짐 적재 상태로 짐칸 포커스 → "트럭은 앞쪽에서 탑승하거나, 휴대폰으로 다른 지역으로
  갈 수 있다고했어." **조건이 선 뒤 한 번만** — 짐칸 앞을 오갈 때마다 뜨면 도배가 된다.

관찰 (플레이모드 Camp, 이벤트 로그):
`PlayerRemarked → 우선 박스를 들어서 옮겨와야겠다.` /
`PlayerRemarked → 먼저 마우스로 클릭해서 송장을 찍어야되.` /
`PlayerRemarked → 트럭은 앞쪽에서 탑승하거나, 휴대폰으로 다른 지역으로 갈 수 있다고했어.`
콘솔 에러·워닝 0.

## S-271 · 발주 2026-08-10 20:07 → ClaudeCode (캠프인데 현재 위치가 빌라촌으로 표시)

요구 (남규님 원문): 지금 캠프에서 탑승했는데 위치가 빌라촌으로 나옴(주황색 아이콘 UI).

진단: S-256에서 화살표를 `currentDistrict` 기준으로 옮겼는데, **캠프로 돌아와도 그 값이 안 비워진다** —
마지막으로 갔던 구역이 남아 화살표가 거기 붙는다.

수용기준: 캠프(및 구역이 아닌 씬)에 있으면 화살표가 **출발 마커 자리**로 돌아온다 ·
출발지 라벨도 "물류캠프"가 된다 · 콘솔 에러·워닝 0.

MDA 판정 (D-070): **강화(역학)** — 지도에서 내 위치가 틀리면 이동 판단이 통째로 어긋난다.

## S-272 · 발주 2026-08-10 20:07 → ClaudeCode (Travel 배너 문구)

요구 (남규님 원문): Travel 씬 TutorialBanner가 약간 불친절한 말투임 →
"원하는 지역을 선택 후 목적지로 출발 버튼을 누르세요"로.

수용기준: Travel 배너 문구가 위 문장이다 · 콘솔 에러·워닝 0.

MDA 판정 (D-070): **무관(문구)**.

## S-273 · 발주 2026-08-10 20:07 → ClaudeCode (Home 기본 가구도 편집 가능하게)

요구 (남규님 원문): Home 씬에 기본 배치 가구들도 다른 가구들과 마찬가지로 클릭해서 옮기거나
삭제하거나 돌릴 수 있게.

수용기준: 처음부터 놓여 있는 가구가 구매 가구와 **같은 조작**(이동·회전·삭제)을 받는다 ·
콘솔 에러·워닝 0.

MDA 판정 (D-070): **강화(역학)** — 같은 화면의 같은 물건이 어떤 건 되고 어떤 건 안 되면 규칙이 없다.

## S-274 · 발주 2026-08-10 20:07 → ClaudeCode (Home 저녁 Point Light)

요구 (남규님 원문): Home 씬에 저녁때 스크린샷과 같이 Point Light 켜 줄 것.
(캡처 인스펙터: position (−2.3, 2.7, 0) · Intensity 3 · Range 10 · No Shadows)

수용기준: 밤/저녁에 Home 실내에 그 조명이 켜진다 · 낮에는 꺼진다 · 재조립해도 유지(빌더 반영) ·
콘솔 에러·워닝 0.

MDA 판정 (D-070): **강화(미학)** — 밤 실내가 그냥 어둡기만 하면 낮밤 전환이 손해로만 보인다.

### S-271~S-274 · 결과 2026-08-10 20:16 (셀프검증 3종 통과 · 실측)

**S-271 지도 현위치**: `GameStateSO.currentDistrict`는 **마지막으로 고른 목적지**라 캠프로 돌아와도
안 비워진다 — S-256에서 그걸 그대로 화살표에 쓴 것이 원인이다. 폰이 **현재 씬**으로 판정하도록
바꿨다(`_hereDistrict`) — 씬이 곧 위치라 어긋날 수 없다. `currentDistrict` 자체는 손대지 않는다
(짐 스폰이 그 값을 쓴다 — 지우면 다른 게 깨진다).
실측: `currentDistrict=[빌라촌]`이 남은 채 캠프에 있을 때 지도 현위치=**(없음=캠프)** ·
출발 라벨=**"출발: 물류캠프"**.

**S-272 Travel 배너**: "원하는 지역을 선택 후 목적지로 출발 버튼을 누르세요"로 교체.

**S-273 기본 가구 편집**: 클릭 판정(`FindPlacedIndex`)은 **배치 대장에 있는 것만** 찾는다 —
빌더가 세운 침대는 대장에 없어 클릭해도 반응이 없었다. 빌더가 마커(`PlacedFurnitureVisual`)를
붙이고, 런타임이 그것을 대장에 등재한다(`AdoptSceneFurniture`, 재입장 멱등).
씬에 실물을 세우는 것은 그대로다 — S-189(씬을 열면 방이 비면 아트가 배치를 못 맞춘다)를 지킨다.
**함정**: `PlacedFurnitureVisual`이 자동 구현 프로퍼티라 **씬 저장에서 값이 통째로 날아갔다**
(실측: `FurnitureId` 빈 값) → 직렬화 필드로 바꿨다.
실측: 재조립 후 `__gb_Bed(fur_bed @(-2.5, 0.0, 0.8) yaw90)` · 플레이 진입 시 배치대장 **1건(fur_bed)**.

**S-274 Home 야간등**: 캡처 인스펙터 값 그대로 빌더에 넣고(위치 (−2.3, 2.7, 0) · 강도 3 · 반경 10 ·
그림자 없음) `NightLamp`로 저녁·밤에만 켠다(규약은 `SignGlow`와 동일 — `DayPhaseChanged` 구독 +
`Start`에서 현재 phase 1회 반영).
실측: 재조립 후 `(-2.3, 2.7, 0.0) 강도3 반경10 그림자=None`.

콘솔 에러·워닝 0.

한계 (정직): **S-273은 침대까지만이다.** 방에 보이는 나머지 가구(책상·의자·화분 등)는 아트 세트
(`set_home`) 안의 배경 오브젝트라 `FurnitureSO` 대응이 없다 — 대장에 올려도 삭제·복원이 성립하지
않는다. 그것들까지 편집하려면 아트 세트에서 꺼내 가구 SO로 등록하는 별도 작업이 필요하다.

## S-275 · 발주 2026-08-10 20:30 → ClaudeCode (가구를 집으면 작아진다)

요구 (남규님 원문): 가구 집으면 조그만해짐. (캡처 — 침대가 원래보다 작게 놓임)

진단 (착수 전 가설): 빌더가 세운 침대는 `localScale *= so.prefabScale`로 배율이 이미 걸려 있다.
집었다 놓으면 `SpawnVisual`이 새로 만드는데 그 경로의 배율 적용이 빌더와 어긋나는 것으로 보인다.

수용기준: 집었다 놓아도 **크기가 그대로**다 · 구매 가구도 종전 크기 유지 · 콘솔 에러·워닝 0.

MDA 판정 (D-070): **강화(미학)** — 만질 때마다 작아지는 물건은 조작 자체를 못 믿게 만든다.

## S-276 · 발주 2026-08-10 20:30 → ClaudeCode (의자·화분도 편집 가능하게)

요구 (남규님 원문): 의자랑 화분도 집어서 옮길 수 있게.

배경: S-273은 침대까지만이었다(아트 세트 배경 오브젝트는 가구 SO 대응이 없다고 판단).
확인해 보니 `Assets/Data/Furniture/`에 `chair`·`fur_plant`가 **실재한다** — 대응이 있으므로 가능하다.

수용기준: 의자·화분이 침대와 같은 조작(이동·회전·삭제)을 받는다 · 재조립해도 유지 ·
콘솔 에러·워닝 0.

MDA 판정 (D-070): **강화(역학)** — 같은 방의 같은 성격 물건이 규칙을 달리하면 안 된다.

## S-277 · 발주 2026-08-10 20:30 → ClaudeCode (가구앱에서 벽지 변경 제거)

요구 (남규님 원문): 가구앱에 벽지 바꾸는 거 없애줘.

수용기준: 폰 가구 앱에 벽지 변경 UI가 없다 · 남은 기능(가구 구매·배치)은 그대로 ·
콘솔 에러·워닝 0.

MDA 판정 (D-070): **무관(정리)**.

추가 (남규님, 같은 건): **장판(바닥) 바꾸는 것도** 가구앱에서 없앨 것. → S-277 범위에 포함.

## S-278 · 발주 2026-08-10 20:30 → ClaudeCode (씬 진입 독백 문구 지정)

요구 (남규님 원문): 씬 첫 진입 SpeechBubble 독백을 아래로 —
빌라촌 "여기가 빌라촌이구나." · 먹자골목 "먹자골목이네 맛있겠다..." ·
언덕주택가 "언덕이 너무 높은데.." · 아파트 "공동현관문 비밀번호는 배달앱에 있다고했어."

배경: 구조는 S-268에서 이미 넣었고(첫 진입 1회·대화 중이면 대기), **문구만** 지정본으로 바꾼다.

수용기준: 네 씬의 문구가 위와 정확히 같다 · 콘솔 에러·워닝 0.

MDA 판정 (D-070): **강화(감정)** — 특히 아파트 문구는 비번을 어디서 보는지까지 알려 준다(안내 겸용).

### S-275~S-278 · 결과 2026-08-10 20:50 (셀프검증 3종 통과 · 실측)

**S-275 가구 축소**: 원인은 **중복 스폰**이었다. 씬에 실물이 서 있는데 대장 항목마다 `SpawnVisual`로
또 만들어, 빌더가 맞춰 둔 모델 오프셋·스케일이 없는 사본이 겹쳐 보였다.
→ 대장 항목이 **씬에 이미 있는 실물과 같으면 그것을 옮겨 쓴다**(스폰하지 않는다).
실측: 대장 5건 · 씬 마커 5개(중복 0) · 침대 스케일 **(2.00, 2.00, 2.00)** — 빌더 값 그대로.

**S-276 의자·화분 편집**: 아트 세트 자식 중 이름이 가구 SO(`Assets/Data/Furniture/*.asset`)와
**정확히 일치하는 것만** 마커를 붙인다 — 대응 SO가 없으면 옮긴 뒤 복원이 안 된다.
실측: `아트 가구 4개를 편집 가능으로 표시` · 편집 대상 5개
(`chair, fur_plant, teddy_bear, fur_bed, desk`).

**S-277 벽지·장판 제거**: 가구앱에서 두 버튼을 걷었다. 팔레트와 상태값은 남겨 뒀다 — 되살리려면
버튼만 다시 달면 된다.

**S-278 씬 진입 독백**: 문구를 지정본으로 교체하고, **안 나오던 원인을 잡았다.**
남규님 지적("씬 독백 문구 안 나왔었어")을 받고 파 보니 두 겹이었다:
1. `PlayerArrivalRemark`는 `GreyboxStageBuilder.BuildPlayer`에 넣었는데 **재조립한 씬에만** 붙었다 —
   실측: Camp·Village 1개 / **FoodStreet·Hillside·Apartment 0개**. 세 씬을 재조립해 전부 1개로 맞췄다.
2. 붙어 있는 씬에서도 안 떴다 — **대화가 끝날 때까지 미루는 로직** 때문이다. 구역 첫 진입에는
   배송 안내가 딸려 와서 그 대기가 사실상 영구가 됐다(실측 시점 `대화중=True` · `pending=True`로 계속 밀림).
   → 대기를 걷었다. 대화창은 화면 하단, 말풍선은 머리 위라 서로 가리지 않는다(캡처로 확인).
   타이머도 `unscaledDeltaTime`으로 바꿔 페이드·정지 중에도 흐른다.
실측: 빌라촌 진입 3초 시점 말풍선 1개 · 문구 `[여기가 빌라촌이구나.]` ·
캡처(`s278_village_remark.png`)에서 머리 위 표시와 하단 대화창이 겹치지 않는 것 확인.

콘솔 에러·워닝 0.

한계 (정직): 기본 가구 **삭제는 세션 안에서만** 유지된다 — 씬을 나갔다 오면 빌더/아트 세트가 실물을
다시 세우므로 되살아난다. 이동·회전은 대장에 남아 재진입에도 유지된다.

## S-279 · 발주 2026-08-10 21:23 → ClaudeCode (가구 미리보기·실물 크기 불일치 + Hillside 고양이 삭제)

요구 (남규님 원문):
① 아직 가구 크기 작아짐 — `Furniture_fur_bed`랑 `Ghost_fur_bed`랑 맞추면 될 것 같다.
② Hillside 씬에 고양이 삭제.

진단 (①): 남규님 힌트가 정확했다. 실물(`SpawnVisual`)은 `so.prefabScale`을 곱하는데
**고스트(`BuildGhost`)는 안 곱한다** — 미리보기가 실물보다 작다(침대는 배율 2.0이라 절반으로 보인다).
S-275에서 중복 스폰은 잡았지만 이 불일치는 남아 있었다.

수용기준: 배치 미리보기와 놓인 결과의 크기가 같다 · Hillside에 고양이가 없다(빌더에서 제거 —
씬에서만 지우면 재조립에 되살아난다) · 콘솔 에러·워닝 0.

MDA 판정 (D-070): **강화(역학)** — 미리보기와 결과가 다르면 배치를 눈으로 못 정한다.

### S-279 · 결과 2026-08-10 21:26 (셀프검증 3종 통과 · 실측)

**① 크기 불일치 — 남규님 힌트가 정확했다.** 실물(`SpawnVisual`)은 `so.prefabScale`을 곱하는데
고스트(`BuildGhost`)만 안 곱했다. 침대는 배율 2.0이라 미리보기가 절반으로 떴다.
S-275에서 중복 스폰은 잡았지만 이 한 줄이 남아 있었다 — 그래서 "아직 작아진다"가 이어졌다.
실측: 고스트 **(2.00, 2.00, 2.00)** = 실물 **(2.00, 2.00, 2.00)** · 일치.

**② Hillside 고양이**: 빌더에서 뺐다(씬에서만 지우면 재조립에 되살아난다).
실측: 재조립 후 `HillsideCat=0` · 이름에 Cat이 들어간 오브젝트 0개.
⚠ **집 고양이(S-059)의 유일한 입수처였다** — `HomeCat`은 데려온 뒤에만 활성이라, 이 씬에서 만나는
경로가 사라지면 그 콘텐츠도 함께 잠긴다. 되살리려면 주석 처리한 `BuildCat` 한 줄만 복구하면 된다.

콘솔 에러·워닝 0.

## S-280 · 발주 2026-08-10 21:36 → ClaudeCode (미니게임이 폰 화면 밖으로 삐져나옴 + 구역 씬에 배너 없음)

요구 (남규님 원문):
① 미니게임이 휴대폰 스크린창이랑 안 맞음 — 딱 맞게 조절.
② 박말순 전화 배너(S-269 ②)가 **Village에서 안 뜬다** — 그 씬에 TutorialBanner가 없어서인 듯.

진단 (②): 맞다. 배너는 `BuildDistrict`가 만들지 않는다 — 캠프·이동·아파트·언덕에만 있고
**빌라촌·먹자골목에는 없다**(S-269 결과에 범위로 기록해 둔 그대로). 전화는 어느 구역에서든
오므로 구역 씬에도 깔아야 한다.

수용기준: 미니게임 판이 폰 화면 안에 정확히 들어간다 · 빌라촌·먹자골목에서도 전화 배너가 뜨고
20초 뒤 사라진다 · 콘솔 에러·워닝 0.

MDA 판정 (D-070): **강화(역학)** — 조작 안내가 화면 밖에 있으면 첫 통화에서 그냥 진다.

### S-280 · 결과 2026-08-10 21:53 (셀프검증 3종 통과 · 좌표 실측)

**① 미니게임 정합**: 미니게임 판은 폰 개구부에 **원본 크기 기준 좌표로 하드코딩**돼 있었다.
S-257·S-259에서 폰을 스케일로 키우면서 그 좌표만 남아 판이 밖으로 삐져나왔다.
→ 폰 배율(`PhoneView.PANEL_SCALE`)을 공개 상수로 올려 **같은 값을 곱한다**.
곱하기만 하면 (좌 2px · 하 10px) 밀려서(실측: 크기는 이미 정확히 일치, 위치만 어긋남)
캔버스 좌표 보정 (+3, +14)을 더했다.
실측 (플레이모드, 월드 코너 비교):
미니게임 좌하 **(1037, 10)** · 우상 **(1266, 418)** / 폰 Screen 좌하 **(1037, 10)** · 우상 **(1266, 418)**
— **네 모서리 완전 일치**.

**② 구역 씬 배너**: `BuildDistrict`가 배너를 안 만들어 빌라촌·먹자골목에는 **뜰 자리 자체가 없었다**
(S-269 결과에 범위로 적어 둔 그대로 — 남규님이 그 구멍을 짚었다). 두 씬에도 깔았다.
실측: Village·FoodStreet 배너뷰 있음 · 빌라촌에서 전화 발생 시
`배너=[박말순 전화를 받고 화면과 동일하게 방향키 빠르게 눌러야한다.] alpha=1.0`.

콘솔 에러·워닝 0.

한계 (정직): **미니게임이 뜬 화면 캡처는 못 남겼다.** `MinigameRequested` 이벤트만으로는 판이
켜지지 않아(실제 통화 흐름을 타야 한다) 캡처 시점에 지도 앱만 떠 있었다.
정합 자체는 위 월드 코너 좌표 일치로 확인했다.

## S-281 · 발주 2026-08-10 22:02 → ClaudeCode (Hillside 손수정 반영 + 미니게임 제목 폰트)

요구 (남규님 원문):
① Hillside 내가 수동 수정함 — 씬 코드에 반영할 것(**머티리얼 및 바닥 사이즈 조정**).
② MinigameCanvas의 Title Font Size **26**으로 수정(휴대폰 밖으로 삐져나감).

수용기준: Hillside를 재조립해도 남규님이 맞춘 머티리얼·바닥 크기가 유지된다(현재 씬 값을 실측해
빌더 정본으로 승격) · 미니게임 제목이 폰 화면 안에 들어온다 · 콘솔 에러·워닝 0.

MDA 판정 (D-070): **무관(배치·가독성)** — 손배치 승격과 문구 넘침 정리.

### S-281 · 결과 2026-08-10 22:11 (셀프검증 3종 통과 · 실측)

**① Hillside 손수정 승격** — 실측해 보니 두 가지가 바뀌어 있었다: 지면 머티리얼이
`GB_HillAsphalt`(빌더 기본은 흙), 전경 판 치수가 pos (37.6, −0.08, −17) · scale (129.43, 0.1, 46).

- **전경 판**: `ExtendGroundForward`의 자동 계산(원본 바운즈 기준)은 아트 세트가 오른쪽으로 더
  넓어진 것을 못 따라가 재조립마다 우측이 잘렸다 → 남규님 치수를 상수로 승격해 덮는다.
- **머티리얼**: 보존 함수(`FindGroundMaterial`, S-226)가 이름이 `GB_`로 시작하면 **통째로 걸렀다** —
  그레이박스 기본값을 제외하려던 필터인데, 남규님이 고른 `GB_HillAsphalt`도 그 접두어라 함께
  걸러져 재조립마다 흙으로 되돌아갔다. **빌더가 만드는 기본값(`GB_HillDirt`) 하나만** 제외하도록 좁혔다.

실측: 아스팔트로 되돌려 저장 → 재조립 → `BaseGround mat=GB_HillAsphalt` ·
`Front scale=(129.43, 0.10, 46.00) mat=GB_HillAsphalt` — **둘 다 유지**.

**② 미니게임 제목 폰트**: 34 → **26**(남규님 지정). 실측: 재조립 후 `fontSize=26`.

콘솔 에러·워닝 0.

## S-282 · 발주 2026-08-10 22:16 → ClaudeCode (Hillside 재수정 반영 + WebGL 배포)

요구 (남규님 원문): Hillside 한 번 더 수정함 — 반영하고 WebGL 배포해 줄 것.

실측 (재조립 전 현재 씬): `__gb_BaseGround` scale **(11.80, 1, 1.2)** — 빌더 값 (12, 1, 1.2)에서
x만 줄었다. 머티리얼(GB_HillAsphalt)·전경 판 치수는 S-281에서 이미 승격한 값 그대로다.

수용기준: 재조립해도 x 11.80이 유지된다 · WebGL 빌드 성공 후 `gh-pages` 갱신 ·
링크 로드 확인 · 콘솔 에러·워닝 0.

배포 범위: 직전 배포(S-234 · 2026-08-10 12:45 · `bc56402a`) 이후 **S-237~S-281 전량**이 미반영이다.

MDA 판정 (D-070): **무관(운영)** — 손배치 승격 + 팀·심사가 보는 창구 갱신.

### S-282 · 결과 2026-08-10 22:35 (셀프검증 3종 통과 · 배포 링크 확인)

**① Hillside 재수정 승격**: 재조립 **전에** 씬을 읽어 바뀐 값을 찾았다 — `__gb_BaseGround` scale
x만 12 → **11.8**(머티리얼·전경 판은 S-281에서 이미 승격한 값 그대로).
실측: 재조립 후 `BaseGround scale=(11.80, 1.00, 1.20) mat=GB_HillAsphalt` ·
`Front scale=(129.43, 0.10, 46.00)` — 셋 다 유지.

**② WebGL 재배포**: 직전 배포(S-234 · 12:45 · `bc56402a`) 이후 **S-237~S-282 전량**을 묶었다.
- 빌드 성공 · `Web.data.unityweb` **76.1MB**(직전 76.3MB — S-236 건물 제외가 계속 유효).
  GitHub 50MB 경고는 뜨지만 100MB 상한 이내라 통과.
- 빌드 중 에디터가 약 7분 무응답(정상 — 빌드가 에디터를 점유한다).
- gh-pages 갱신(`7061bafc`) → Pages 반영까지 약 1분 → 새 파일 서빙 확인
  (`content-length=79790378`, `Last-Modified 13:30 GMT`).
- 링크 로드: 로딩바 100% · 데이터 79.79MB 수신 · **타이틀 정상 표시**.
- **버전 각인이 화면에 없다** — S-238(릴리스 빌드에서 각인 숨김)이 실제로 동작함을 여기서 처음
  확인했다(그 발주의 미검증 항목이 이번 배포로 닫혔다).

콘솔 에러·워닝 0.

## S-283 · 발주 2026-08-10 22:51 → ClaudeCode (WebGL에서 벚꽃 배경이 검게 나온다)

요구 (남규님 원문): 에디터에서는 엔딩씬 `BlossomPetalEffect`의 배경 투명이 정상인데,
**WebGL 빌드에서는 배경이 검정**으로 나온다. 원인 확인하고 고칠 것.

진단 (착수 전): `BlossomPetalEffect`는 `Shader.Find`로 셰이더를 찾아 **런타임에 머티리얼을 생성**하고
`_SURFACE_TYPE_TRANSPARENT` 키워드를 코드로 켠다. 에디터는 모든 배리언트가 살아 있어 정상이지만,
**빌드는 실제로 쓰이는 배리언트만 남긴다** — 프로젝트 어느 에셋도 그 조합(투명 파티클)을 들고 있지
않으면 스트립되어 불투명 배리언트로 떨어지고, 알파가 무시돼 텍스처의 투명부가 검게 찍힌다.
이것이 "에디터만 정상"의 전형적 형태다.

수용기준: WebGL 빌드에서 꽃잎 배경이 투명하다 · 에디터 회귀 없음 ·
**빌드로 실제 확인**한다(에디터 플레이만으로 판정하지 않는다) · 콘솔 에러·워닝 0.

MDA 판정 (D-070): **강화(미학)** — 캠프·엔딩의 벚꽃은 계절감의 핵심 연출이다.
검은 사각형이 떠다니면 그 장면이 무너진다.

### S-283 · 결과 2026-08-10 23:15 (셀프검증 3종 통과 · 빌드 배포까지)

**원인은 셰이더 배리언트 스트리핑이었다.** `BlossomPetalEffect`가 `Shader.Find` + `new Material`로
**런타임에** 머티리얼을 만들고 투명 키워드를 코드로 켰다. 에디터는 모든 배리언트가 살아 있어
정상이지만, 빌드는 실제로 쓰이는 배리언트만 남긴다 — 프로젝트 어느 에셋도 그 조합을 들고 있지
않아 스트립되었고, 알파가 무시돼 배경이 검게 찍혔다. "에디터만 정상"의 전형이다.

- 꽃잎 머티리얼을 **에셋 정본**으로 만들었다(`Assets/Data/Greybox/GB_BlossomPetal.mat`) —
  에셋으로 존재해야 배리언트가 수집된다. 런타임은 그것을 인스턴스화해 쓰고, 없을 때만 종전 생성으로 떨어진다.
- **Alpha Clipping을 켰다 (남규님 지적)**. 이 부분이 컸다 — 블렌드만으로는 부족하다는 것을
  S-245·S-246에서 이미 겪었는데(그때도 실해법은 Alpha Clipping이었다) 여기서 또 놓쳤다.
  Cutoff는 **0.1**로 낮춰 수명 끝 페이드아웃이 뚝 끊기지 않게 했다.
- 빌더 두 곳(캠프·촬영용 District 1)이 머티리얼을 주입한다. 실측: 캠프 재조립 후
  꽃잎 효과 6개 **전부 주입**.

배포: 빌드 성공 → gh-pages(`d036ec78`) → Pages 서빙 확인(`content-length=79799019`) →
링크 로드 정상(타이틀 표시, 데이터 79.8MB 수신). 콘솔 에러 0.

한계 (정직): **배포본에서 꽃잎이 실제로 투명한지는 남규님 눈으로 봐 주셔야 한다.**
꽃잎은 캠프·엔딩 장면에 있어 타이틀 화면에서는 확인되지 않고, 관제가 배포본을 플레이해
그 장면까지 도달하지는 못했다. 원인·수정은 위와 같고 에디터 회귀는 없다.
