...
## S-206 · 발주 2026-08-08 03:20 → 정수 공장 (EditMode 테스트 확충 — 미커버 4영역)

요구 (남규님 지시): "게임 개발 완료를 위해 다른 사람 작업과 충돌나지 않는 범위에서 수정·개선할 수
있는 부분" 정리 후 **①②③ 전부 진행** 승인. 본 건은 ①.

배경 (git 실측): `Assets/Scripts/Editor/Tests/`는 테스트 8파일 46테스트. 커밋 이력 전량 정수 —
관제·아트 접촉 0이라 충돌 위험이 구조적으로 없다. 커버된 축은 정산·바코드·가방·주문판·숙련도·
해금·빚. `lateCount` 이중가산은 회귀테스트(`DeliverySettleTests.cs:94`)로 봉인된 뒤 재발이 죽었다 —
같은 방식을 미커버 축으로 넓힌다.

대상 4영역 (현재 테스트 0):
1. **마감·지각 판정** — `WorldDeadlineManager` (경고 발화 시점 · 초과 시 실패 전이)
2. **스태미나·온도 패널티** — `PlayerStatusManager` (716줄, 폭염/한파 구간 드레인·회복)
3. **날씨 전이** — `WorldWeatherManager` (1163줄, 상태 전이표·낮밤 분리)
4. **엔딩 발동 조건** — `WorldEndingManager` (빚 청산 판정 → 시퀀스 진입 여부)

제약:
- **프로덕션 코드 수정 0을 목표로 한다.** 테스트를 위해 접근자를 뚫어야 하면 그 사실을 결과에
  적고, 관제 파일(`Managers/*`)은 최소 변경으로 막는다 — 남규님이 같은 파일에서 S-205를 시공 중.
- 기존 `TestSupport.cs` 헬퍼를 재사용한다. 새 헬퍼는 두 번째 사용처가 생길 때만 추출(YAGNI).
- 테스트가 현재 동작을 "옳다"고 못 박는 게 목적이 아니다. 발주서·대장에 적힌 **규칙**을 기준으로
  쓰고, 코드가 규칙과 다르면 테스트를 코드에 맞추지 말고 **결함으로 보고**한다.

수용기준: 새 테스트가 EditMode에서 전부 통과 · 기존 46건 무회귀 · 컴파일 0에러 0워닝 ·
프로덕션 코드 변경분이 있으면 파일·줄·이유 명시

MDA 판정 (D-070): **무관** — 재미 3축(A1 긴장감·A2 성취감·A3 생활감) 어디에도 직접 기여하지 않는
품질 인프라다. 무관 축이라 후순위가 원칙이나, 남규님 명시 지시로 착수한다. 근거는 재미가 아니라
**회귀 비용** — 코어루프가 S-200번대에서 계속 수리되는 중이고, 수리마다 다른 축이 깨지는지 알
방법이 현재 46건뿐이다.

### S-206 결과 2026-08-08 03:53 (정수 공장 · 리드 ~33분 · feature/jjs-s206-au029-au030, base=main)

신규 4파일 + `TestSupport.cs` 확장. **프로덕션 코드 수정 0.**

**EditMode 46 → 90건 (신규 44, 전량 통과 · failed 0)**

| 파일 | 건 | 잠근 규칙 |
|---|---|---|
| `DeadlineJudgeTests.cs` | 9 | 경고 구간 진입·지각 전이·**중복 억제**(경고/실패 각 1회)·**배치 완료건 마감 면제(S-073 ①)**·완료 통지 시 기록 해제·빈 적재 무판정 |
| `StaminaPenaltyTests.cs` | 10 | 폭염·한파·태풍·상자당 패널티 표 · 해소 음료 유효 구간 · **상한 차감 모델**(S-088 ④) · 하한 10 · 변화 시에만 이벤트 |
| `WeatherRollTests.cs` | 13 | 기온표 6종+기본값 · 추첨 범위 · **예보 승계(S-058)** · 같은 날 재추첨 금지 · 맑음 고정 3종(인트로 S-184 / 먹자골목 S-189 / 엔딩 S-202) |
| `EndingTriggerTests.cs` | 12 | **오발동 방지 5종** · 대열 규칙(박말순 선두·호감도 내림차순·정원 6·도감 충원·중복 방지) |

**설계 판단 3건**
1. **엔딩은 음성 경로만 잠갔다.** 시퀀스 본체가 코루틴이라 EditMode에서 안 돈다. 대신 "발동하면
   안 되는 조건"(엔딩 기재생·빚 잔존·독백 전 캠프 도착·무관 씬)을 전부 걸었다 — 엔딩이 잘못 터지면
   하루가 통째로 날아가는 쪽이라 음성 경로가 곧 회귀 위험이다. 이 판단을 파일 주석에 남겼다.
2. **스태미나 드레인은 범위 밖.** `Update()`가 `Mouse.current`·`EventSystem`·프레임 dt에 물려 있어
   EditMode에서 재현 불가. 대신 그 아래 순수 규칙인 `TickPenalties`/`EffectiveStaminaMax`를 잠갔다.
3. **날씨는 시각 경로를 우회했다.** `SetWeather`가 파티클·구름을 건드리는데 미주입(null)은 전부
   조기 반환한다. 단 `_clouds`만 null 검사 없이 순회하므로 빈 배열을 주입했다. 태풍은 `StartCoroutine`을
   타므로 테스트에서 SetWeather 인자로 쓰지 않았다.

**TestSupport 확장 2개** — `GetField`(판정 후 내부 플래그 확인 · 사용처 8곳) ·
`InvokeStatic`(private static 규칙 함수 · 사용처 4곳). 둘 다 두 번째 사용처가 실재해 추출했다(YAGNI 준수).

**관찰 (실측)**
- `unity-cli editor refresh --compile` → **0에러 0워닝** · `unity-cli console --type error,warning` → `[]`
- `unity-cli test` → **total 90 / passed 90 / failed 0 / skipped 0** (기존 46건 무회귀)
- 신규 44건 전부 **첫 실행에서 통과** — 코드 쪽 결함 적발분 0건. 즉 이번 4영역의 현행 동작은
  대장에 적힌 규칙과 일치한다(테스트를 코드에 맞춰 느슨하게 쓴 것이 아니라, 규칙을 먼저 적고 맞은 것).
