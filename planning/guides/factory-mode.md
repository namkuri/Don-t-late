...
- **발주 커밋 ≠ 납품 커밋 (D-060 — 전 모드 공통, 2026-07-28 남규님 지시로 명시)**: 셀프 재량 작업(자발 4조건)이나
  발주를 쪼개 이어갈 때도, 대장에 발주/계획을 append한 `[발주] AU-0XX: 제목` 커밋을 **작업 시작 전에 먼저 push**
  (📦 훅 알림 = 착수 예고). 결과+코드는 별도 납품 커밋. 한 커밋에 뭉치면 착수 예고·리드타임 계측이 무너진다.
...
- 받기: 서버 #git(GitHub 연동) 채널 팔로우 — 발주(📦)·결과(✅)·INBOX(🔔)·PR이 흐른다.
- 보내기(자기 커밋도 알림 태우려면): 채널 웹훅 URL 발급 후 1회 `git config dontlate.webhook <URL>`.
