...
| 9 | **ElevenLabs** | **BGM+SFX 겸용 (오디오 1순위)** | audio | ⬜ | API 키 발급 → `ELEVENLABS_API_KEY` 환경변수 → MCP 로컬 등록 (`pipelines/audio.md` §0) | #10 Freesound → 무음+최소 신디 |
...
- [ ] #8~10: 클립 1개 확보→라이선스 기록→임포트 관통
      (2026-07-21 실사: **#8 Suno 공식 API 부재 확인** → 1순위를 #9로 교체. 공식 MCP `elevenlabs/elevenlabs-mcp`가
      `compose_music`·`text_to_sound_effects`를 모두 제공해 **BGM·SFX를 #9 하나로 덮는다.**
      반입·후공정 파이프라인 `scripts/audio/` 납품 완료 — **잔여는 API 키 발급뿐**)
