...
|---|---|---|---|---|---|---|
| 1 | **unity-cli** (CoplayDev unity-mcp) | 씬·컴파일·임포트·스왑·빌드 | logic-unity, build-deploy | ⬜ | Unity 패키지 설치 + Claude Code MCP 등록 | **없음 — 치명. 최우선 검증** |
...
| 11 | **git + GitHub** | 소스·커밋 증거(제출 규정) | build-deploy | ⬜ | gh auth login, 원격 저장소 생성 | 로컬 git만(제출 리스크 — 조기 해결) |
| 12 | **GitHub Pages** | WebGL 배포·공개 링크 | build-deploy | ⬜ | Pages 설정 or gh-pages 브랜치 | 대안 정적 호스팅 (규정: 웹 접근이면 됨) |
| 13 | 스키마 검증 스크립트 | 데이터 검증 (내장) | content-data | ⬜ | scripts/validate_schema.py 준비기간 제작 | 수동 JSON 검토(느림) |

## 준비 기간 검증 의무 (레일 체크리스트에 편입)
- [ ] #1 unity-cli: 빈 프로젝트에서 컴파일·씬 조작 1회 관통 — **가장 먼저**
- [ ] #11+#12: 빈 씬 WebGL 빌드 → Pages 배포 → 링크 열림 1회 관통
