...
```yaml
frozen: true                   # Q1 동결 2026-07-21 (D-037) — 기존 줄 수정=사람 게이트+훅 차단, 직교 추가만 허용
pipeline_levels:               # ⭐ 셰이더 토끼굴 방지 캡
  L1_필수: "네이티브 해상도 렌더 → 풀스크린 스냅 셰이더(스크린 UV를 480×270 그리드로 양자화 — Pixelate.shader) + 텍스처 Point 필터"   # 실구현 반영 D-011·D-025 (사람 룩 승인). 저해상 RT 방식은 WebGL 성능 필요 시 예비안
