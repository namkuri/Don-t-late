...
flowchart TD
    ...
    %% ── 부족한 곳 (미완·블로커) ──
    G1["엔딩 시퀀스 완성 (S-104)<br/>빚 0→독백→박말순 마중→작별→잊지마 크레딧→타이틀"]:::done
...
| 🔴 부족 | 심사 전 반드시 메워야 하는 구멍 | 아트/애니 대기·밸런스 미튜닝 (엔딩은 S-104로 해소 ✅) |

**관제 소견 (2026-07-29 S-104 반영)**: 루프 골격(🟢)은 원안(🔵)을 초과 달성했고,
마지막 완결 구멍이던 **엔딩이 닫혔다** — 빚 0 → 독백 → 박말순+이웃 마중 → 작별 →
"늦지마→잊지마" 크레딧 → 타이틀 (S-104, 감사 2연속 지적 해소). 남은 🔴는 아트 스왑
대기·밸런스 미튜닝. 마감(2026-08-10)까지 우선순위는 🔴 → 🟡(날씨 오디오·구역 조명) 순.
