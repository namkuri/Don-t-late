...
flowchart TD
    ...
    G5["WebGL 재배포 필요<br/>공개 링크가 구버전 (최신 기능 미반영)"]:::gap
    G6["심사 전 치트 제거<br/>Y키 날씨·은행 +1000 테스트 버튼"]:::gap
...
| 🔴 부족 | 심사 전 반드시 메워야 하는 구멍 | **엔딩 조건·WebGL 재배포·치트 제거**·아트/애니 대기 |

**관제 소견**: 루프 골격(🟢)은 원안(🔵)을 초과 달성했다 — 남은 승부처는 🔴 중
"엔딩 조건 + WebGL 재배포 + 밸런스"의 게임으로서의 완결, 그리고 🟡 중 체감이 큰
"날씨 오디오(AU-018)·구역 조명 아이덴티티"다. 마감(2026-08-10)까지 우선순위는 🔴 → 🟡 순.
